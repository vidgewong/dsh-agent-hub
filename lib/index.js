...
var brand = llm;
var callIdBrand = brand.ToolCallId ?? brand.CallId;
if (!callIdBrand) {
  throw new Error(
    "@deepseek-ai/dsh-llm exports neither ToolCallId nor CallId; this dsh-llm version is not supported by dsh-omniloop"
  );
}
var CallId = callIdBrand;
...
function chunkIndex(chunk) {
  return "index" in chunk ? chunk.index : void 0;
}
function rebaseChunkIndices(chunks, offset) {
  if (offset === 0) return [...chunks];
  return chunks.map((chunk) => {
    const index = chunkIndex(chunk);
    return index === void 0 ? chunk : { ...chunk, index: index + offset };
  });
}
function maxChunkIndex(chunks) {
  ...
  for (const chunk of chunks) {
    ...
    if (max === void 0 || index > max) max = index;
  }
  return max;
}
...
// src/driver-core/missing-sdk.ts
var CLAUDE_CODE_SDK = {
  engine: "Claude Code",
...
function missingSdkError(sdk, cause) {
  return new Error(
    `the ${sdk.engine} engine requires "${sdk.pkg}", which is not installed in this profile. Install it into the profile that runs dsh (a bare "pnpm add" elsewhere will not be resolved), then restart dsh:
  dsh plugin --profile <name> add ${sdk.pkg}@${sdk.version}
Or pick another engine for this session \u2014 "in-process" needs no SDK. (${String(cause)})`
  );
}
...
function backendDiagnostic(env, backend) {
  const active = BACKEND_ENV_GROUPS.find((group) => env[group.selector] !== void 0);
  if (active === void 0) {
    return `claude-code: no provider backend configured (backend ${backend}); the CLI will fall back to its own login state. Set ANTHROPIC_BASE_URL for a relay, or CLAUDE_CODE_USE_BEDROCK/CLAUDE_CODE_USE_VERTEX, via the plugin's \`env\` config or the environment dsh was launched from.`;
  }
...
function modelDiagnostic(model, provider, env) {
  if (model === void 0 || provider === void 0) return void 0;
  const active = BACKEND_ENV_GROUPS.find((group) => env[group.selector] !== void 0);
  if (active === void 0 || active.id === "relay") return void 0;
  return `claude-code: selected model "${model}" (provider ${provider}) may not be served by the ${active.id} backend; sending it as-is.`;
}
function claudeQueryOptions(spec, controller) {
  ...
  if (routing !== void 0) report(routing);
  if (spec.providerEnv === void 0) {
    const mismatch = modelDiagnostic(spec.model, spec.provider, childEnv);
    if (mismatch !== void 0) report(mismatch);
  }
  return {
    ...
    // Forward subagent text and thinking blocks so the driver can render a
    // nested transcript inside the parent step. Without it the SDK yields
    // only heartbeat tool_use/tool_result frames for subagents, and the
    // parent session shows the Task tool call with no inner visibility.
...
// src/engine-claude/provider-env.ts
var BEDROCK_PROVIDER_IDS = /* @__PURE__ */ new Set(["amazon-bedrock"]);
var ANTHROPIC_PROVIDER_IDS = /* @__PURE__ */ new Set(["anthropic"]);
function readProviderProfile(ctx, provider) {
...
function mapRouteToEnv(provider, profile, key) {
  ...
  if (BEDROCK_PROVIDER_IDS.has(provider)) {
    return {
      ...
      ...profile.baseURL === void 0 ? {} : { ANTHROPIC_BEDROCK_BASE_URL: profile.baseURL },
...
async function deriveProviderEnv(ctx, provider, model) {
  ...
  if (routeEnv === void 0) return void 0;
  const modelDefaults = model === void 0 || model === "" ? {} : {
  ...
  return {
    env,
    diagnostic: `claude-code: routing to dsh provider "${provider}"${profile.baseURL === void 0 ? "" : ` at ${profile.baseURL}`} (credential ${ref})${model === void 0 || model === "" ? "" : `; subagent model ${model}`}${extra.length === 0 ? "" : `; route env ${extra.join(", ")}`}`
  };
}
...
var SubagentChildSession = class {
  session;
  id;
  options;
  inbox;
  ...
  seeded = false;
  constructor(loopCtx, parentAgent, sessions, agents, label, provider, prompt) {
    ...
    const engineRecords = loopCtx.get("loopEngineRecords");
    if (engineRecords !== void 0) {
      void engineRecords.remember({ id: childId, ...parentHeader.cwd === void 0 ? {} : { cwd: parentHeader.cwd } }, provider).catch(() => {
      });
    }
  }
  get status() {
  ...
  seedPrompt(prompt) {
    ...
    if (prompt === void 0 || prompt.length === 0) return;
    this.seeded = true;
...
var ClaudeCodeAgent = class {
  ...
  send(message, target, wakeup) {
    if (wakeup && this.liveSink !== void 0 && this.phase.kind === "running" && !this.phase.abort.signal.aborted && this.liveSink(message)) {
      return;
    }
  ...
  async resolveModel(signal) {
    ...
    if (this.config.model !== void 0) {
      return { model: this.config.model, provider: void 0, source: "config" };
    }
  ...
  async step() {
    ...
    const diagnostics = [];
    const sessionStore = this.loopCtx.get("sessions");
    const agentRegistry = this.loopCtx.get("agents");
    const childSessions = /* @__PURE__ */ new Map();
    const taskMeta = /* @__PURE__ */ new Map();
    const skippedTasks = /* @__PURE__ */ new Set();
    let warnedNoStore = false;
    const getChildSession = (parentToolUseId) => {
      ...
      if (skippedTasks.has(parentToolUseId)) return void 0;
    ...
    try {
      ...
      const options = claudeQueryOptions({
        ...
        env: this.config.env,
        ...derived === void 0 ? {} : { providerEnv: derived.env },
      ...
      const ownCallIds = /* @__PURE__ */ new Set();
      let blockOffset = 0;
      let maxSeenIndex = -1;
      let lastRawStart;
      const rollStep = () => {
      ...
      const streamedReasoning = () => [...reasoningByIndex.entries()].sort((a, b) => a[0] - b[0]).map(([, text]) => ({ type: "reasoning", text }));
      this.liveSink = (injected) => {
        ...
        switch (message.type) {
          case "stream_event": {
            if (message.parent_tool_use_id !== null) {
              ...
              if (child !== void 0) {
                const event2 = message.event;
                if (event2.type === "content_block_start") {
                  if (child.lastRawStart !== void 0 && event2.index <= child.lastRawStart) {
                    child.blockOffset = child.maxSeenIndex + 1;
                  }
                  child.lastRawStart = event2.index;
                }
                const chunks2 = rebaseChunkIndices(mapStreamEvent(event2, child.toolCalls), child.blockOffset);
                const highest2 = maxChunkIndex(chunks2);
                if (highest2 !== void 0 && highest2 > child.maxSeenIndex) child.maxSeenIndex = highest2;
            ...
            if (event.type === "content_block_start") {
              if (lastRawStart !== void 0 && event.index <= lastRawStart) {
            ...
            if (highest !== void 0 && highest > maxSeenIndex) maxSeenIndex = highest;
            for (const chunk of chunks) {
          ...
          case "assistant": {
            if (message.parent_tool_use_id !== null) {
              const child = getChildSession(message.parent_tool_use_id);
              if (child !== void 0) {
                const mapped2 = mapAssistantMessage(message.message);
                const messageId2 = message.message.id;
                const reasoningOnlyPending2 = child.pendingContent.length > 0 && child.pendingContent.every((block) => block.type === "reasoning");
                if (child.pendingMessageId !== void 0 && messageId2 !== child.pendingMessageId && !reasoningOnlyPending2) {
                  child.flushAssistant();
                }
                if (child.stepFlushed) child.rollStep();
                child.pendingMessageId = messageId2;
                let content2 = mapped2.content;
                const haveReasoning2 = child.pendingContent.some((block) => block.type === "reasoning") || content2.some((block) => block.type === "reasoning");
                if (child.reasoningByIndex.size > 0 && !haveReasoning2) {
                  content2 = [...[...child.reasoningByIndex.entries()].sort((a, b) => a[0] - b[0]).map(([, text]) => ({ type: "reasoning", text })), ...content2];
                }
                if (content2.some((block) => block.type === "reasoning")) child.reasoningByIndex.clear();
                child.pendingContent = [...child.pendingContent, ...content2];
                if (mapped2.usage !== void 0) child.pendingUsage = mapped2.usage;
                child.pendingModel = mapped2.model;
                if (mapped2.toolCalls.length > 0) {
                  child.flushAssistant();
                  for (const call of mapped2.toolCalls) {
            ...
            const reasoningOnlyPending = pendingContent.length > 0 && pendingContent.every((block) => block.type === "reasoning");
            if (pendingMessageId !== void 0 && messageId !== pendingMessageId && !reasoningOnlyPending) {
              flushAssistant();
            }
            if (stepFlushed) rollStep();
            pendingMessageId = messageId;
            let content = mapped.content;
            const haveReasoning = pendingContent.some((block) => block.type === "reasoning") || content.some((block) => block.type === "reasoning");
            ...
            if (mapped.usage !== void 0) pendingUsage = mapped.usage;
            ...
            if ("subtype" in message && message.subtype === "task_started") {
              ...
              if (task.tool_use_id !== void 0) {
                ...
                taskMeta.set(task.tool_use_id, {
                  ...
                  ...task.prompt === void 0 ? {} : { prompt: task.prompt }
                });
...
import { createToolResultMessage as createToolResultMessage2 } from "@deepseek-ai/dsh-llm";
...
import { LlmError as LlmError3, createAssistantMessage as createAssistantMessage3, createUserMessage as createUserMessage3, errorChain as errorChain3 } from "@deepseek-ai/dsh-llm";
...
// src/router.ts
function shadowAgents(agents, capture) {
...
function shadowSystemPrompt(systemPrompt, claim) {
  return new Proxy(systemPrompt, {
    get(target, prop, receiver) {
      if (prop === "variable") {
        return (name2, provider) => {
          if (!claim(name2)) return () => {
          };
          return target.variable(name2, provider);
        };
      }
      const value = Reflect.get(target, prop, receiver);
      return typeof value === "function" ? value.bind(target) : value;
...
var MAX_RESERVATIONS = 64;
var LoopEngineRouter = class {
  constructor(options) {
    this.options = options;
  }
  options;
  ...
  claim(sessionId) {
    ...
    if (reserved === void 0) return this.options.engineForNewSession();
    this.reservations.delete(sessionId);
    return reserved;
  }
  /**
  ...
  withSetup(engine, setup) {
    const decorate = this.options.decorateSetup;
    if (decorate === void 0) return setup;
    return (agentCtx) => {
  ...
  async createAgent(ownerCtx, options) {
    ...
    await this.options.remember(
      { id: options.sessionId, ...options.meta?.cwd === void 0 ? {} : { cwd: options.meta.cwd } },
      engine
    );
...
var SIDECAR_SUFFIX = ".loop-engine.json";
function fallbackDir() {
  return join3(resolveDshHome(), "loop-engine");
}
function fallbackPathFor(sessionId) {
  return join3(fallbackDir(), `${encodeURIComponent(sessionId)}.json`);
}
...
var EngineRecordStore = class {
  ...
  pathFor(meta) {
    ...
    return located === void 0 ? fallbackPathFor(meta.id) : `${located.path}${SIDECAR_SUFFIX}`;
  }
  /**
  ...
  async recall(sessionId) {
    ...
    if (located !== void 0) candidates.unshift(located);
    for (const path of candidates) {
      ...
      if (engine !== void 0) return engine;
    }
    return "in-process";
  }
  /**
   ...
   */
  async locateByInspect(sessionId) {
    ...
    try {
      ...
      return located === void 0 ? void 0 : `${located.path}${SIDECAR_SUFFIX}`;
    } catch {
      return void 0;
    }
  }
   ...
   */
  async collectOrphans(limit = 200) {
    ...
    for (const entry of entries.slice(0, limit)) {
      ...
      try {
        ...
        await rm(join3(dir, entry), { force: true });
        removed += 1;
      }
    }
    return removed;
...
// src/rpc.ts
var LOOP_ENGINE_RPC_CHANNEL = "/loop-engine";
...
function engineField(payload) {
  const value = stringField(payload, "engine");
  return LOOP_ENGINE_IDS.find((id) => id === value);
}
function createEngineRpcHandler(target) {
  return async (endpoint, payload) => {
    const sessionId = stringField(payload, "sessionId");
    if (sessionId === void 0) return badRequest(`${endpoint}: sessionId must be a non-empty string`);
    switch (endpoint) {
      case "bind": {
...
function installEngineRpc(connection, register, target) {
...
function discoverUserSlashCommands() {
  ...
  for (const entry of entries.sort()) {
    ...
    const path = join4(userCommandsDir(), entry);
    let raw;
    try {
...
function userCommandsDir() {
  return join4(homedir(), ".claude", "commands");
}
...
var ClaudeCodeSkillProvider = class {
  ...
  async list(options) {
    ...
    if (cwd !== void 0) {
      ...
      await collectSkillsDir(join5(projectRoot, ".claude", "skills"), CLAUDE_CODE_RANK, candidates);
      await collectClaudeMd(projectRoot, candidates);
    }
    await collectSkillsDir(join5(homedir2(), ".claude", "skills"), CLAUDE_CODE_USER_RANK, candidates);
...
async function collectSkillsDir(skillsDir, rank, candidates) {
  ...
  for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
    const entryPath = join5(skillsDir, entry.name);
    const info = await stat(entryPath).catch(() => void 0);
    if (info === void 0) continue;
    if (info.isDirectory()) {
      const path = join5(entryPath, "SKILL.md");
...
var CodexSkillProvider = class {
  ...
  async list(options) {
    ...
    const userPath = join7(homedir3(), ".codex", "AGENTS.md");
...
function piAgentDir() {
  ...
  return join8(homedir4(), ".pi", "agent");
}
var PiSkillProvider = class {
  ...
  async list(options) {
    ...
    if (cwd !== void 0) {
      ...
      for (const dir of projectDirs) {
        await this.collectSkillsDir(join8(dir, ".pi", "skills"), PI_SKILL_PROJECT_RANK, candidates);
      }
    }
    const userAgentDir = piAgentDir();
    const userContext = join8(userAgentDir, "AGENTS.md");
    if (await fileNonEmpty(userContext)) candidates.push(this.agentsCandidate([userContext], PI_AGENTS_USER_RANK));
    await this.collectSkillsDir(join8(userAgentDir, "skills"), PI_SKILL_USER_RANK, candidates);
    if (this.control.signal.aborted) return [];
    return candidates;
  }
  async get(candidate, _options) {
  ...
  async collectSkillsDir(skillsDir, rank, candidates) {
    ...
    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      const entryPath = join8(skillsDir, entry.name);
      const info = await stat3(entryPath).catch(() => void 0);
      if (info === void 0) continue;
      if (info.isDirectory()) {
        const path = join8(entryPath, "SKILL.md");
...
var inject = ["agents", "systemPrompt"];
var Config4 = z5.object({
  ...
  model: z5.string(),
  backend: z5.union(CLAUDE_CODE_BACKENDS.map((id) => z5.const(id))),
...
function mountBaseLoop(ctx, loopCtx, mount) {
...
function registerEngineSkills(engine, agentCtx) {
...
function apply(ctx, config) {
  ...
  ctx.inject(["connection", "webServer"], (connectionCtx) => {
    installEngineRpc(
      connectionCtx.get("connection"),
      (install, label) => {
        connectionCtx.effect(install, label);
      },
      {
        reserve: (sessionId, engine) => {
          router.reserve(sessionId, engine);
        },
  ...
  ctx.inject(["llm", "tools"], (injectedCtx) => {
  ...
  void records.collectOrphans().catch((error) => {
