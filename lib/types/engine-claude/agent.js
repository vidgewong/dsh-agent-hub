...
import { PluginInbox } from "../driver-core/inbox.js";
import { PluginAssistantStream } from "../driver-core/assistant-stream.js";
...
import { CLAUDE_CODE_SDK, missingSdkError } from "../driver-core/missing-sdk.js";
...
class SubagentChildSession {
    ...
    seeded = false;
    constructor(loopCtx, parentAgent, sessions, agents, label, provider, 
    ...
    newAssistantStream(turn, step) {
        this.assistantStreamStarted = false;
        return new PluginAssistantStream(this.id, ++this.assistantAttemptCounter, () => ++this.assistantStreamRevision, turn, step, this.dispatch);
    }
    ...
    flushAssistant() {
        ...
        const append = () => this.session.append('assistant/message', {
            turn: this.turn,
            step: this.step,
    ...
    async step() {
        ...
        const getChildSession = (parentToolUseId) => {
            if (sessionStore === undefined) {
                if (!warnedNoStore) {
                    warnedNoStore = true;
                    diagnostics.push(`agent "${this.id}": no session store in scope; subagent transcripts were dropped`);
                }
                return undefined;
            }
            ...
            if (skippedTasks.has(parentToolUseId))
                return undefined;
            ...
            if (child === undefined) {
                ...
                child = new SubagentChildSession(this.loopCtx, this, sessionStore, agentRegistry, meta?.description, PROVIDER, meta?.prompt);
        ...
        try {
            ...
            // it up. Streaming input keeps ONE query alive while the turn is still
            // being answered: the serialized history is the first stream message, and
            // `send()` pushes every later mid-turn message into the same stream, so
            // Claude receives it immediately.
            //
            // The stream is closed on the FIRST `result`, which is the turn's
            // completion boundary. We deliberately do NOT count pushed messages
            // against results: the CLI's push→result ratio is not fixed. A message
            // injected while Claude is mid-turn — especially during a subagent — is
            // often FOLDED into the running turn and shares its single `result`
            // (verified against claude 2.1.x). A counter that expected one result per
            // push then stalled above zero, never closed the stream, and hung the
            // whole turn until a manual stop aborted it. Closing on the first result
            // never truncates work already handed to the CLI: `endInput()` sends EOF
            // but the child drains every stdin line queued before it, so an injection
            // pushed just before the close still runs and streams its own trailing
            // `result`, which the loop below keeps consuming until the query settles.
            // Messages that arrive after the close fall through to the inbox for the
            // next turn, exactly as they would with no live query at all.
             ...
             */
            let assistantStream = new PluginAssistantStream(this.id, ++this.assistantAttemptCounter, () => ++this.assistantStreamRevision, turn, step, this.dispatch);
            ...
            const rollStep = () => {
                ...
                this.session.append('step/start', { turn, step });
                stepFlushed = false;
                assistantStream = new PluginAssistantStream(this.id, ++this.assistantAttemptCounter, () => ++this.assistantStreamRevision, turn, step, this.dispatch);
                assistantStreamStarted = false;
                toolCalls.clear();
            ...
            // into the live stream, so Claude sees it during the turn. The CLI may
            // fold it into the running turn or open a fresh one — either way the
            // driver does not track it: the stream closes on the first `result`, and
            // any turn the injection opens streams and settles before the query does.
            // Injection during a coalescing API turn first flushes the pending
            // assistant so the injected user turn lands after the text it is
            // responding to, then rolls to a fresh step for the reply.
            ...
            for await (const message of query) {
                ...
                switch (message.type) {
                    case 'stream_event': {
                        ...
                        if (message.parent_tool_use_id !== null) {
                            ...
                            if (child !== undefined) {
                                ...
                                if (highest !== undefined && highest > child.maxSeenIndex)
                                    child.maxSeenIndex = highest;
                                for (const chunk of chunks) {
                                    if (!child.assistantStreamStarted) {
                                        child.assistantStream.start();
                                        child.assistantStreamStarted = true;
                                    }
                        ...
                        if (message.parent_tool_use_id !== null) {
                            ...
                            if (child !== undefined) {
                                for (const result of mapToolResults(message.message)) {
                                    if (!child.ownCallIds.has(String(result.source.callId)))
                                        continue;
                    ...
                    case 'result': {
                        // Do NOT close child sessions here. A `result` frame is the
                        // boundary of one CLI turn, not of the whole subagent tree: when
                        // the parent runs Task subagents the CLI emits a `result` as soon
                        // as the parent's own turn settles, then keeps streaming the
                        // subagents' frames (verified against the real CLI — subagent
                        // transcript frames arrive AFTER the first `result`). A mid-turn
                        // injection makes the CLI settle an extra interim `result` to
                        // answer it, so a single turn commonly carries several results
                        // while the same subagents keep running under stable
                        // `parent_tool_use_id`s. Closing + clearing `childSessions` on any
                        // result therefore tore down subagents that were still working
                        // (they flipped to a settled dot), and because the loop keeps
                        // consuming, their later frames re-created fresh child sessions
                        // under the same tool ids — the "subagent finished, then an
                        // identical one restarts" behavior. Child sessions are instead
                        // closed once, in the `finally` below, when the query has fully
                        // settled and every subagent frame has been consumed.
                        //
                        ...
                        // The turn Claude was answering is complete. Stop accepting live
                        // input and close the stream so the SDK settles the query and the
                        // CLI exits. We close on the FIRST result rather than counting one
                        // result per pushed message: the CLI often folds a mid-turn
                        // injection into the running turn and answers both under a single
                        // result, so a counter would stall above zero and hang the turn.
                        // Closing here does not truncate a just-pushed injection — the CLI
                        // drains stdin queued before the EOF, so any turn it opens still
                        // streams and settles below before the query ends. `close()` and
                        // clearing the sink are idempotent, so a second result (when the
                        // injection did open its own turn) passes through harmlessly.
            ...
            this.liveSink = undefined;
            input.close();
            // The single close point for every subagent child session. The query has
            // fully settled here (completion, error, or abort) and every subagent
            // frame has been consumed, so each child holds its complete transcript;
            // closing now settles all of them exactly once. This is deliberately not
            // done on `result` frames, which recur within one turn and would tear
            // down still-running subagents mid-flight (see the `result` case).
            for (const child of childSessions.values())
                child.closeTurn();
