 ...
 *
 * @module dsh-omniloop/engine-claude/mapping
 */
 ...
 */
export function mapToolResults(message) {
...
function toolResultContent(content) {
    ...
    if (Array.isArray(content)) {
        for (const block of content) {
            ...
            if (candidate === null || candidate.type !== 'text')
                continue;
...
/** A chunk's block index, or undefined for chunks carrying no block coordinate. */
function chunkIndex(chunk) {
    return 'index' in chunk ? chunk.index : undefined;
}
/**
 ...
 */
export function rebaseChunkIndices(chunks, offset) {
    if (offset === 0)
        return [...chunks];
    return chunks.map((chunk) => {
        const index = chunkIndex(chunk);
        return index === undefined ? chunk : { ...chunk, index: index + offset };
    });
}
/**
 ...
 */
export function maxChunkIndex(chunks) {
    let max;
 ...
 */
export function mapStreamEvent(event, toolCalls) {
    switch (event.type) {
