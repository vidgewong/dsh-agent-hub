...
import { applyManagedBlock, currentEngineOf, } from "./patch-manager.js";
 ...
 */
export async function syncManagedBlock(path, engine) {
    const current = await readPatchOrUndefined(path);
    if (current !== undefined && currentEngineOf(current) === engine)
        return false;
