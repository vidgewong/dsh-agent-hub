...
import { ClaudeCodeLoop, CLAUDE_CODE_BACKENDS, CLAUDE_CODE_PERMISSION_MODES } from "./engine-claude/loop.js";
import { CodexLoop, CODEX_APPROVAL_POLICIES, CODEX_SANDBOX_MODES } from "./engine-codex/loop.js";
import { PiLoop } from "./engine-pi/loop.js";
import { applyManagedBlock } from "./patch-manager.js";
import { loopEngineSettingsNamespace, LOOP_ENGINE_SETTINGS_SCHEMA, } from "./settings.js";
import { LoopEngineRouter, shadowSystemPrompt } from "./router.js";
import { EngineRecordStore } from "./engine-record.js";
import { installEngineRpc } from "./rpc.js";
import { CLAUDE_CODE_COMMANDS, discoverUserSlashCommands } from "./commands.js";
import { ClaudeCodeSkillProvider } from "./skills.js";
import { CodexSkillProvider } from "./engine-codex/skills.js";
import { PiSkillProvider } from "./engine-pi/skills.js";
...
export function resolvePatchPath(config) {
    if (config.patchPath !== undefined && config.patchPath !== '')
        return config.patchPath;
    return join(resolveDshHome(), 'profiles', config.profile ?? 'web', config.patchFilename ?? 'cordis.patch.yml');
}
...
export async function writePatchFile(path, text) {
 ...
 */
export function writePatchFileSync(path, text) {
 ...
 */
export async function syncManagedBlock(path) {
 ...
 */
export function mountBaseLoop(ctx, loopCtx, mount) {
    void import('@deepseek-ai/dsh-agent-loop').then(({ default: AgentLoop }) => {
 ...
 */
export function registerEngineSkills(engine, agentCtx) {
...
export function apply(ctx, config) {
    ...
    const records = new EngineRecordStore(() => ctx.get('sessionPersistence'));
    ...
    ctx.inject(['connection', 'webServer'], (connectionCtx) => {
        installEngineRpc(connectionCtx.get('connection'), (install, label) => { connectionCtx.effect(install, label); }, {
    ...
    const mount = (engine, plugin) => {
        const fiber = plugin();
        void fiber.then(() => undefined, (error) => {
    ...
    const engineCtx = (engine) => ctx.extend({
        ...
        systemPrompt: shadowSystemPrompt(ctx.systemPrompt, (variable) => {
            if (claimedVariables.has(variable))
                return false;
