...
import { applyManagedBlock, currentEngineOf, renderManagedBlock, } from "./patch-manager.js";
...
const install = (ctx, fail) => {
    ...
    for (const engine of LOOP_ENGINE_IDS) {
        ...
        if (reborn !== applied)
            fail(`managed-block round trip for ${engine} is not a fixed point`);
        if (engine === 'in-process' && applied !== seed)
            fail('in-process engine must leave the file text unchanged');
        if (engine !== 'in-process' && currentEngineOf(renderManagedBlock(engine)) !== engine)
            fail(`${engine} block must read back as the ${engine} engine`);
    }
    /* v8 ignore stop */
};
/**
