 ...
 */
import { applyManagedBlock, hasManagedBlock, MANAGED_BLOCK_BEGIN, MANAGED_BLOCK_END, renderManagedBlock, } from "./patch-manager.js";
...
const install = (ctx, fail) => {
    ...
    if (applyManagedBlock(applied) !== applied)
        fail('managed-block application is not a fixed point');
    if (!hasManagedBlock(applied))
        fail('managed block must be present after application');
    // The block must always disable the base loop: this plugin's router owns the
    // single AgentFactory slot in every configuration.
    if (!renderManagedBlock().includes('- id: agent-loop'))
        fail('managed block must disable the base agent-loop row');
    if (!renderManagedBlock().includes('disabled: true'))
        fail('managed block must disable, not merely target, the base row');
    ...
    if (upgraded !== applied)
        fail('a legacy engine-tagged block must upgrade to the permanent block');
    /* v8 ignore stop */
};
/**
