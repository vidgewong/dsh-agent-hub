 ...
 * durable session log as the sole source of model context. dsh-omniloop
 ...
 * subprocess service.
 *
 * @module dsh-omniloop/engine-codex
 */
 ...
 */
export class CodexLoop extends Service {
    ...
    prepare(ownerCtx, id, options, session, callerSignal, parentAgent) {
    ...
    async setupAndPublish(ownerCtx, id, preparation, agentOptions, setup, signal, source, parentAgent, afterSetup) {
    ...
    async createAgent(ownerCtx, options) {
        ...
        const published = this.setupAndPublish(ownerCtx, options.sessionId, preparation, options.agentOptions ?? {}, options.setup, options.signal, 'startup', options.parentAgent);
    ...
    async resumeWith(ownerCtx, persistence, options) {
        ...
        try {
            ...
            return await this.setupAndPublish(ownerCtx, id, preparation, options.agentOptions ?? {}, options.setup, options.signal, 'resume', options.parentAgent, (session) => this.appendUnstoredSuffix(capturedStored, session));
        }
        catch (error) {
