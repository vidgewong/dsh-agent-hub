...
import { engineColor, engineLabelKey } from "./engine-visuals.js";
...
export function LoopEngineComposerSelect(props) {
    ...
    if (!rpc.available) {
        return (_jsxs("span", { style: frozen, title: t('boundNotice'), children: [engine !== undefined && _jsx("span", { style: dot(engineColor(engine)) }), label] }));
    }
    ...
    return (_jsx(Menu, { open: open, onClose: () => { setOpen(false); }, items: ENGINE_OPTIONS.map(option => ({
            id: option.value,
            label: (_jsxs("span", { style: { display: 'inline-flex', alignItems: 'center', gap: 8 }, children: [_jsx("span", { style: dot(engineColor(option.value)) }), t(option.key)] })),
        })), selectedId: engine, onSelect: onSelect, align: "start", portal: true, getAnchorRect: () => triggerRef.current?.getBoundingClientRect() ?? null, anchor: (_jsxs("button", { type: "button", ref: triggerRef, "aria-haspopup": "menu", "aria-expanded": open, disabled: busy, style: busy ? triggerBusy : trigger, title: notice, onClick: () => { setOpen(!open); }, children: [engine !== undefined && _jsx("span", { style: dot(engineColor(engine)) }), label, _jsx(IconChevronDownOutline14, { size: 14 })] })) }));
}
